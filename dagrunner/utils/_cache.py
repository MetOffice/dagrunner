...
class _PickleCache:
    def __init__(self, node_id, verbose=False):
        self._node_id = str(node_id).replace(os.sep, "_")
        self._verbose = verbose
        cache_dir = CONFIG["dagrunner_runtime"].get("cache_dir", None)
    ...
    def cache_available(self):
        return self._pickle_filepath and os.path.exists(self._pickle_filepath)
    ...
    def load(self):
        if self._pickle_filepath and os.path.exists(self._pickle_filepath):
            if self._verbose:
